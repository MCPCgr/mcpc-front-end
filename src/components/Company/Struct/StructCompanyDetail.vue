  ...
  <div>
    <a-menu>
      <a-menu-item>
        ...
        <StructCompany v-if="struct.isCompany" :struct="struct"></StructCompany>

        <h2 class="text-center" v-else>{{ struct.struct_name }}</h2>


      </a-menu-item>
      <a-menu-item>
        Утас: {{ company.company_phone }}
      </a-menu-item>
      <a-menu-item v-if="struct.struct_type === 'branch' || struct.isCompany">
        <div class="max-w-md whitespace-normal">

          Хаяг: {{ company.company_address }}
        </div>
      </a-menu-item>
      <a-sub-menu key="sub1" v-if="!struct.child_company_id">
        <template #icon>
          <inline-svg
