<template>
  ...
  >
    <template v-for="m in menu.filter(
        (mItem) => permissions.permissions[mItem.id] && permissions.permissions[mItem.id].show === true)" :key="getPath(m)" >
    ...
    const updateMenu = () => {
      ...
      if(menu_list){
        ...
        if(route.path.startsWith("/document")){
          parentKeys.push( '0304460c-f1fb-da47-a8d9-c28dcc72e2d8')

          selectedKeys.value.push("/document/document")
        }

        if(route.path.startsWith("/plan")){


          selectedKeys.value.push("/plan")
        }
