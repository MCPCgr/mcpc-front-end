...
export default {
  ...
  methods: {
    ...
    async setupFirebase(attempt = 0) {
      ...
      try {
        ...
        try {
          ...
          if (currentToken) {
            try {
              await axios.get(`https://mcpc.mn/lambda/notify/token/${this.$props.userID}/${currentToken}`);
            } catch (error) {
    ...
    async getUnseenNotification() {
      try {
        const response = await axios.get('https://mcpc.mn/lambda/notify/new/' + this.$props.userID);
    ...
    async getAllNotification() {
      try {
        const response = await axios.get('https://mcpc.mn/lambda/notify/all/' + this.$props.userID);
        this.count = 0;
        this.notifications = response.data.notifications;
      } catch (error) {
    ...
    async setSeen(id, link) {
      try {
        ...
        if (currentNotif && currentNotif.seen === 0) {
          const response = await axios.get('https://mcpc.mn/lambda/notify/seen/' + id);
