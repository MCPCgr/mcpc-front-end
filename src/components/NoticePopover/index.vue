<template>
  ...
  >
        <span class="action header-notice">
            <a-badge :count="count">
    ...
    <template #content>
      ...
      <div class="notif-container">
        <a-list v-if="notifications?.length">
          <a-list-item v-for="notif in notifications"
                       :key="notif.id"
                       :class="notif.seen === 0 ? 'notif-unseen' : ''"
          >
            <a-list-item-meta
              :title="`${notif.title}`"
            ...
            >
              <template #description>
                <span class="notif-content">
                    ...
                    <span :class="notif.seen === 0 ? 'notif-date-unseen' : 'notif-date'">{{ timeAgo(notif.created_at) }}</span>
                </span>
              </template>
            </a-list-item-meta>
            <span v-if="notif.seen === 0" class="notif-dot"></span>
          </a-list-item>
        </a-list>
      ...
      <a class="notif-footer" href="javascript:void(0)" @click="getAllNotification">
...
export default {
  ...
  methods: {
    ...
    async setupFirebase(attempt = 0) {
      ...
      try {
        ...
        try {
          ...
          if (currentToken) {
            try {
              await axios.get(`https://api.amjilt.com/main/lambda/notify/token/${this.$props.userID}/${currentToken}`);
            } catch (error) {
    ...
    async getUnseenNotification() {
      try {
        const response = await axios.get('https://api.amjilt.com/main/lambda/notify/new/' + this.$props.userID);
    ...
    async getAllNotification() {
      try {
        const response = await axios.get('https://api.amjilt.com/main/lambda/notify/all/' + this.$props.userID);
        this.count = 0;
        this.notifications = response.data.notifications;
      } catch (error) {
    ...
    async setSeen(id, link) {
      try {
        ...
        if (currentNotif && currentNotif.seen === 0) {
          const response = await axios.get('https://api.amjilt.com/main/lambda/notify/seen/' + id);
