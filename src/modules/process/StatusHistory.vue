    ...
    <ol v-else :class="`relative  ${!smallSignature ? '' : ''}`">
      <li v-for="(item, index) in statusHistories" :key="index"
        ...
        <h3 class="flex items-center mb-1 font-semibold  text-gray-900 dark:text-white">
          {{ getStatusLabel(item.status_action) }}
        </h3>
        <time class="block mb-2 text-sm font-normal leading-none text-gray-700 dark:text-gray-500">
          {{ t("process.date") }}: {{ item.is_done || item.status_type === 'END'  || item.status_type === 'CANCEL' ? $dateTime(item.created_at) : t("process.ActionPending") }}
        </time>
        ...
        <div class="border rounded p-3 mt-2" style="border-color: #0b4da3" v-if="item.signature && !smallSignature">
          <table style="width:100%; margin: 0; padding: 0; color: #0b4da3">
            <tr>
              ...
              <td
                ...
                <span
                  v-else> {{ $t('process.ESigned') }}: {{ item.signature.lastname ? item.signature.lastname.charAt(0).toUpperCase() : '' }}.{{ item.signature.firstname }} <br/> {{ $t('process.datetime') }}: {{
                    $dateTime(item.created_at)
                  }}</span>
        ...
        <div class="" style="" v-else-if="item.status_type !== 'VOTE' && item.emp_id">
          <table style="width:100%; margin: 0; padding: 0; ">
            <tr>
              <td style="text-align: left; font-family: Arial; font-size: 12px; vertical-align: middle"
                  class="text-gray-700">
               <span v-html="getEmployeesByID(item.emp_id)"></span>
                ...
                <tr>
                  ...
                  <td
                    ...
                    <div v-if="vote.signature_image">
                      ...
                      <span v-if="vote.signature_image.type === 'text'">{{ $t('process.ESigned') }} {{ $t('process.datetime') }}: <br>{{
                      ...
                      <div v-else>
                                <span
                                  > {{ $t("process.voted") }}: <span v-html="getEmployeesByID(item.emp_id)"></span> <br/>
                                  {{ $t('process.datetime') }}: {{
                                    $dateTime(vote.signature_date)
                                  }}</span>
                      </div>
...
function getEmployeesByID(empID) {
  const index = employees.value.findIndex(employee => employee.id === empID);
  if(index >= 0){
