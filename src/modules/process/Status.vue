<template>
  <div class="flex justify-end items-center gap-3">
    ...
    <div
      v-for="(nextStep, index) in statusHistory.filter(h=>!h.is_done && h.status_id !== currentStep.id && h.step_data)"
      :key="index"
                ...
                <div v-if="nextStep.status_type !== 'VOTE' && currentStep.attrs.subject.object_type !== 'VOTE'">
                  ...
                  <div class="w-full">
                    ...
                    <div v-if="nextStep.subject_type === 'TO_ORG_STRUCT'">
                      <OrgStructEmployeeSelector :disabled="!nextStep.is_subject_changeable" v-model:company_id="nextStep.company_id" v-model:struct_id="nextStep.struct_id" v-model:job_id="nextStep.job_id" v-model:emp_id="nextStep.emp_id" @change="(d)=>changeEmp(nextStep.status_id, d)" mode="TO_ORG_STRUCT"/>
                    </div>
                    <div v-if="nextStep.subject_type === 'TO_ORG_JOB'">
                      <OrgStructEmployeeSelector :disabled="!nextStep.is_subject_changeable" v-model:company_id="nextStep.company_id" v-model:struct_id="nextStep.struct_id" v-model:job_id="nextStep.job_id" v-model:emp_id="nextStep.emp_id"  @change="(d)=>changeEmp(nextStep.status_id, d)" mode="TO_ORG_JOB"/>
                    </div>
                    <div v-if="nextStep.subject_type === 'TO_ORG_EMPLOYEE'">
                      <OrgStructEmployeeSelector :disabled="!nextStep.is_subject_changeable" v-model:company_id="nextStep.company_id" v-model:struct_id="nextStep.struct_id" v-model:job_id="nextStep.job_id" v-model:emp_id="nextStep.emp_id" @change="(d)=>changeEmp(nextStep.status_id, d)"  mode="TO_ORG_EMPLOYEE"/>
                    </div>
...
const globalSubjectHandler = (payload) => {
  ...
  props.statusHistory.forEach(h=>{
    if(!h.is_done && h.status_id !== props.currentStep.id && h.step_data){
      ...
      if(payload.company_id || payload.company_id === null){
        h.company_id = payload.company_id;
      }
