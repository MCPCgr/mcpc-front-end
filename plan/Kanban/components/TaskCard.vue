<template>
  <div class="bg-white shadow rounded px-3 pt-3 pb-1  hover:bg-gray-100 task" @click="edit">
    ...
    <div class="flex justify-between items-start">
      ...
      <div class="flex-grow flex justify-between items-center">
        ...
        <span class="text-gray-700 font-sans tracking-wide text-sm ml-2">{{task.task}}</span>
    ...
    <div class="flex mt-4 justify-between items-center" v-if="task.start_date">
      <span class="text-xs text-gray-600">{{$date(task.start_date) }}<span> - {{$date(task.end_date) }}</span></span>

      <badge v-if="task.type" :color="badgeColor">{{task.type}}</badge>
    </div>

    <div class="ltr:text-right rtl:text-left text-xs text-slate-600 dark:text-slate-300 mb-1 font-medium mt-3" v-if="task.progress > 0 && task.progress_calculate_type !== 'not_calculate'">
