...
export function findTaskById(tasks, id) {
  for (let task of tasks) {
    ...
    if (task.children.length > 0) {
      const foundTask = findTaskById(task.children, id);
      if (foundTask) {
        return foundTask;
      }
    }
...
export function walkLevel(nodes, level) {

  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    node.level = `${level}${i + 1}`;

    node.text = getGanttTaskText(node.level, node.name);
    walkLevel(node.children, `${node.level}.`);
  }
}

export function getGanttTaskText(level, name) {
